"""Supervised dataset helpers for Step 5 S2-S4."""
...
def get_step5_condition_dim() -> int:
    return int(BASE_CONDITION_BUNDLE_DIM)


def _build_step5_condition_bundle_from_values(
    *,
    soluble: int,
    temperature: float,
    phi: float,
...
def _resolve_d_water_paths(base_config: Dict[str, Any]) -> List[Path]:
    ...
    if missing:
        raise FileNotFoundError(f"Step 5 D_water CSVs not found: {missing}")
    return paths


def load_step5_water_dataset(
    base_config: Dict[str, Any],
) -> pd.DataFrame:
    """Load the Step 5 D_water dataset with explicit missing continuous fields."""
...
class ConditionScaler:
    """Min-max scaler for Step 5 continuous conditions."""
...
def build_condition_scaler(resolved: ResolvedStep5Config) -> ConditionScaler:
...
def build_step5_supervised_frames(resolved: ResolvedStep5Config) -> Dict[str, pd.DataFrame]:
    ...
    water_df = add_d_water_split_column(
        load_step5_water_dataset(resolved.base_config),
...
def build_condition_bundle(
    ...
    *,
    ...
    """Build the raw Step 5 condition vector and metadata."""
    ...
    bundle = _build_step5_condition_bundle_from_values(
        soluble=int(soluble),
...
def build_inference_condition_bundle(
    *,
    ...
    """Build the canonical Step 5 inference condition bundle."""
    return _build_step5_condition_bundle_from_values(
        soluble=int(soluble),
...
def build_inference_condition_bundle_from_target_row(
    ...
    *,
    ...
    """Build the canonical Step 5 inference condition bundle from a target-row mapping."""
...
class Step5ConditionalDataset(Dataset):
...
def step5_collate_fn(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    """Collate Step 5 supervised samples into one batch."""

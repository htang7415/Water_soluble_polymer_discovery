"""Supervised Step 5 model-construction and optimization utilities."""
...
from .dataset import get_step5_condition_dim


class Step5AuxHeads(nn.Module):
...
def build_s2_components_from_step1(
    ...
    *,
    ...
    Optional[Step5AuxHeads],
    Path,
    Dict[str, object],
]:
    """Load Step 1 backbone and wrap it into the Step 5 conditional model."""
    ...
    try:
        ...
        raise FileNotFoundError(
            "Step 5 supervised training requires an existing Step 1 backbone checkpoint and tokenizer. "
            f"model_size={resolved.model_size!r}, split_mode={resolved.split_mode!r}. "
            f"Original error: {exc}"
        ) from exc
    hidden_size = int(pretrained_backbone.hidden_size)
    condition_dim = get_step5_condition_dim()
    ...
    s2_cfg = (run_cfg or resolved.step5).get("s2", {})
    ...
    aux_heads = Step5AuxHeads(hidden_size).to(device) if variant == "mt" else None
    return tokenizer, diffusion, aux_heads, checkpoint_path, finetune_info


def load_step5_checkpoint_into_modules(
    *,
...
def build_optimizer_and_scheduler(
    ...
    *,
    ...
    if warmup_schedule != "linear":
        raise ValueError(f"Unsupported Step 5 warmup_schedule: {warmup_schedule}")
    if lr_schedule != "constant_after_warmup":
        raise ValueError(f"Unsupported Step 5 lr_schedule: {lr_schedule}")

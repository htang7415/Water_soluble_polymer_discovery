#!/usr/bin/env python
"""Timed Step 6_2 conditional sampler probe for per-family DiT checks."""
...
from src.step6_2.conditional_sampling import create_conditional_sampler, sample_conditional_with_class_prior
from src.step6_2.config import load_step6_2_config
from src.step6_2.dataset import build_inference_condition_bundle
...
def _build_temp_step62_config(
    *,
    ...
    step6_cfg = payload.setdefault("step6_2", {})
    step6_cfg["c_target"] = str(c_target)
    step6_cfg["enabled_runs"] = [str(name) for name in enabled_runs]
...
def _build_temp_base_config(
    *,
    ...
    step6_cfg = chi_training_cfg.setdefault("step6_class_inverse_design", {})
    target_key = str(c_target).strip().lower()
    if override_length_prior_min_tokens is not None:
        overrides = dict(step6_cfg.get("decode_constraint_length_prior_min_tokens_overrides", {}) or {})
        overrides[target_key] = int(override_length_prior_min_tokens)
        step6_cfg["decode_constraint_length_prior_min_tokens_overrides"] = overrides
    if override_length_prior_max_tokens is not None:
        overrides = dict(step6_cfg.get("decode_constraint_length_prior_max_tokens_overrides", {}) or {})
        overrides[target_key] = int(override_length_prior_max_tokens)
        step6_cfg["decode_constraint_length_prior_max_tokens_overrides"] = overrides
    if override_class_match_min_request_size is not None:
        overrides = dict(step6_cfg.get("decode_constraint_class_match_min_request_size_overrides", {}) or {})
        overrides[target_key] = int(override_class_match_min_request_size)
        step6_cfg["decode_constraint_class_match_min_request_size_overrides"] = overrides
    if override_terminal_star_anchor is not None:
        overrides = dict(step6_cfg.get("decode_constraint_backbone_template_terminal_star_anchor_overrides", {}) or {})
        overrides[target_key] = bool(override_terminal_star_anchor)
        step6_cfg["decode_constraint_backbone_template_terminal_star_anchor_overrides"] = overrides
    if override_backbone_template_max_templates is not None:
        overrides = dict(step6_cfg.get("decode_constraint_backbone_template_max_templates_overrides", {}) or {})
        overrides[target_key] = int(override_backbone_template_max_templates)
        step6_cfg["decode_constraint_backbone_template_max_templates_overrides"] = overrides
    if override_cycle_backbone_template_cores_across_targets is not None:
        overrides = dict(
            step6_cfg.get("decode_constraint_cycle_backbone_template_cores_across_targets_overrides", {}) or {}
        )
        overrides[target_key] = bool(override_cycle_backbone_template_cores_across_targets)
        step6_cfg["decode_constraint_cycle_backbone_template_cores_across_targets_overrides"] = overrides
    return _write_temp_yaml(payload)


def _find_latest_conditional_checkpoint(results_dir: Path, family: str) -> Optional[Path]:
    family_root = results_dir / "step6_2_inverse_design" / "polymer" / str(family).strip().lower()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Timed Step 6_2 conditional sampler probe.")
    parser.add_argument("--config", default="configs/config6_2_smoke.yaml")
    ...
    parser.add_argument(
        "--output_dir",
        default="results_small/step6_2_inverse_design/polymer/_dit_timed_probes",
    )
    args = parser.parse_args()

    device = str(args.device or ("cuda" if torch.cuda.is_available() else "cpu"))
    temp_config_path = _build_temp_step62_config(
        ...
        for target_row_index in target_row_indices:
            ...
            for setting_name, overrides in settings.items():
                ...
                for key, value in overrides.items():
                    resolved_variant.step6_2[key] = value

#!/bin/bash
# Submit S0 baseline + per-family Step 6_2 HPO jobs + dependent Step 6_3 compare on NREL.
...
if [ "$INCLUDE_S0" = "1" ]; then
  ...
  if [ "$s0_complete" -eq 0 ]; then
    jid_s0=$(sbatch \
      ...
      --gres=gpu:1 \
      --chdir "$PROJECT_ROOT" \
      --wrap "bash scripts/run_step6_2_nrel.sh \"$MODEL_SIZE\" \"S0_raw_unconditional\"")

#!/bin/bash
# Submit Step 6_2 runs plus dependent Step 6_3 comparison on Euler.
...
for run_name in "${RUN_NAMES[@]}"; do
  ...
  jid=$(sbatch \
    ...
    --gres=gpu:1 \
    --chdir "$PROJECT_ROOT" \
    --wrap "bash scripts/run_step6_2_euler.sh \"$MODEL_SIZE\" \"$run_name\"")
...
jid63=$(sbatch \
  "${STEP63_ARGS[@]}" \
  --wrap "bash scripts/run_step6_3_euler.sh \"$MODEL_SIZE\" \"$RUNS\"")

echo "Submitted Step 6_2 + Step 6_3 chain on Euler:"
if [ ${#submitted_job_ids[@]} -gt 0 ]; then

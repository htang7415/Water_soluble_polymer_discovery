...
def _save_figures(
    ...
    if stage_counts[0] > 0:
        ...
        ax.invert_yaxis()
        ax.set_xlabel("Count")
        ax.set_title("Step 5 target-polymer screening funnel")
    ...
    if not candidate_df.empty and not target_poly_df.empty:
        joint_selected = ((target_poly_df["soluble_hit"] == 1) & (target_poly_df["property_hit"] == 1)).astype(int)
    ...
    if not target_poly_df.empty and {"target_chi", "chi_pred_target"}.issubset(target_poly_df.columns):
        ...
        ax.set_title("Step 5 selected polymers: target vs predicted χ")
        fig.tight_layout()
        ...
        sns.histplot(margin_vals.dropna(), bins=min(20, max(8, len(sel) // 6)), color="#e45756", ax=ax)
        ax.axvline(0.0, color="#474747", linewidth=1.0, linestyle=":", label="Requirement boundary")
        ax.set_xlabel("Signed property margin")
        ax.set_ylabel("Count")
        ax.set_title("Step 5 selected polymers: property margin distribution")
        ax.legend(frameon=False)
        fig.tight_layout()
...
def main(args):
    ...
    if training_canonical is None or score_outputs is None:
        raise RuntimeError("Step 5 failed to build any candidate pool from fresh Step 2 resampling.")
    ...
    if int(target_poly_summary["target_count_selected"]) == 0:
        raise RuntimeError(
            "Step 5 could not find any target polymers that satisfy the solubility + property requirements "
            f"after {len(attempt_rows)} fresh sampling attempts."
        )
    ...
    pool_summary = {
        ...
        "step2_resampling_step_dir": attempt_manifests[0].get("step2_resampling_step_dir", ""),
        "step2_resampling_generated_csv": attempt_manifests[0].get("step2_resampling_generated_csv", ""),
        "step2_resampling_summary_csv": attempt_manifests[0].get("step2_resampling_summary_csv", ""),
    ...
    summary = {
        ...
        # Backward-compatible summary key used by downstream rollups: true target success.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Unsupported override. Step 5 always launches a fresh Step 2 sampling run.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum fresh Step 2 sampling attempts to accumulate before giving up (default: chi_training.step5_inverse_design.sampling_attempts_max or 5)",
    )

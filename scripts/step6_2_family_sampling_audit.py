#!/usr/bin/env python
"""Quick Step 6_2 family sampling audit for iterative robustness checks."""
...
def _resolve_available_families(config_path: str, families_arg: Optional[str]) -> List[str]:
    ...
    step6_cfg = payload.get("step6_2", {})
    return [str(name).strip().lower() for name in step6_cfg.get("available_target_classes", [])]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Quick Step 6_2 family sampling audit.")
    ...
    parser.add_argument("--output_dir", default="results_small/step6_2_inverse_design/polymer/_family_quality_audit")

...
PY
  ...
  jid=$(sbatch \
    ...
    --gres=gpu:1 \
    --chdir "$PROJECT_ROOT" \
    --wrap "FRESH_STUDY=\"$FRESH_STUDY\" bash scripts/run_step6_2_hpo_euler.sh \"$MODEL_SIZE\" \"$family\"")
...
jid63=$(sbatch \
  "${STEP63_ARGS[@]}" \
  --wrap "ALLOW_PARTIAL=\"1\" bash scripts/run_step6_3_euler.sh \"$MODEL_SIZE\" \"$COMPARE_RUNS\"")

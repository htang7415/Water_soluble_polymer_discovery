#!/bin/bash
# Submit Step 4, then Step 5 and Step 5_1 on NREL.
# Assumes Step 0-3 outputs already exist.
# Usage:
#   bash scripts/submit_steps4_5_nrel.sh <model_size> <polymer_family> [runs_csv]
...
if [ -z "$POLYMER_FAMILY" ]; then
  echo "Usage: bash scripts/submit_steps4_5_nrel.sh <model_size> <polymer_family> [runs_csv]"
  exit 2
fi

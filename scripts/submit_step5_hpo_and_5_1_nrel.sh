...
if [ "$INCLUDE_S0" = "1" ]; then
  ...
  if [ "$s0_complete" -eq 0 ]; then
    jid_s0=$(sbatch \
      ...
      --gres=gpu:1 \
      --chdir "$PROJECT_ROOT" \
      --wrap "bash scripts/run_step5_nrel.sh \"$MODEL_SIZE\" \"S0_raw_unconditional\"")
...
PY
  ...
  jid=$(sbatch \
    ...
    --gres=gpu:1 \
    --chdir "$PROJECT_ROOT" \
    --wrap "FRESH_STUDY=\"$FRESH_STUDY\" bash scripts/run_step5_hpo_nrel.sh \"$MODEL_SIZE\" \"$family\"")
...
jid51=$(sbatch \
  "${STEP51_ARGS[@]}" \
  --wrap "ALLOW_PARTIAL=\"1\" bash scripts/run_step5_1_nrel.sh \"$MODEL_SIZE\" \"$COMPARE_RUNS\"")

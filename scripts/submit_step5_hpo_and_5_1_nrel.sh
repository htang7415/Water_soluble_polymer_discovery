#!/bin/bash
# Submit S0 baseline + per-family Step 5 HPO jobs + dependent Step 5_1 compare on NREL.
# Usage:
#   bash scripts/submit_step5_hpo_and_5_1_nrel.sh <model_size> <polymer_family> [study_families_csv]
...
if [ "$INCLUDE_S0" = "1" ]; then
  ...
  if [ "$s0_complete" -eq 0 ]; then
    jid_s0=$(sbatch \
      ...
      --gres=gpu:1 \
      --chdir "$PROJECT_ROOT" \
      --wrap "bash scripts/run_step5_nrel.sh \"$MODEL_SIZE\" \"$POLYMER_FAMILY\" \"S0_raw_unconditional\"")
...
PY
  ...
  jid=$(sbatch \
    ...
    --gres=gpu:1 \
    --chdir "$PROJECT_ROOT" \
    --wrap "FRESH_STUDY=\"$FRESH_STUDY\" bash scripts/run_step5_hpo_nrel.sh \"$MODEL_SIZE\" \"$POLYMER_FAMILY\" \"$family\"")
...
jid51=$(sbatch \
  "${STEP51_ARGS[@]}" \
  --wrap "ALLOW_PARTIAL=\"1\" bash scripts/run_step5_1_nrel.sh \"$MODEL_SIZE\" \"$POLYMER_FAMILY\" \"$COMPARE_RUNS\"")

echo "Submitted Step 5 HPO + Step 5_1 chain on NREL:"
if [ ${#submitted_job_ids[@]} -gt 0 ]; then
